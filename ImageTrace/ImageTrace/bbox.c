...
static void initInterval(interval_t *i, double min, double max) {
  i->min = min;
  i->max = max;
}

/* initialize the interval to [x, x] */
void singleton(interval_t *i, double x) {
  initInterval(i, x, x);
}

/* extend the interval to include the number x */
void extend(interval_t *i, double x) {
  if (x < i->min) {
    i->min = x;
  } else if (x > i->max) {
    i->max = x;
...
int in_interval(interval_t *i, double x) {
...
void segment_limits(int tag, dpoint_t a, dpoint_t c[3], dpoint_t dir, interval_t *i) {
  switch (tag) {
  case POTRACE_CORNER:
